...
var (
	...
	accPosPattern = `\{"eventTime":"[^"]+","exchange":"[^"]+","positionMode":"[^"]+","accountId":"([^"]+)","transactionId":"[^"]+".*\}`
	accPosReg     = regexp.MustCompile(accPosPattern)
)
...
func filterAccData(inputFile string, account string) error {
	return scanFileAsLine(inputFile, func(line string) error {
		...
		_, jsonData, match := matchAccountAndPosition(line, account, accPosReg)
